...
export class ScenarioRunner {
    ...
    async runScenario(scenario) {
        ...
        try {
            ...
            const response = await this.client.request(scenario.request.method, scenario.request.params);
            ...
            const validationErrors = this.validateExpectations(scenario, response, upstreamInvoked);
     ...
     */
    validateExpectations(scenario, response, upstreamInvoked) {
        const errors = [];
        ...
        if (expect.success !== undefined) {
            ...
            if (expect.success && !isSuccess) {
                errors.push(`Expected success but got error: ${response.error?.message || "unknown"}`);
            }
            if (!expect.success && isSuccess) {
        ...
        if (expect.errorCode !== undefined && response.error) {
            if (response.error.code !== expect.errorCode) {
                errors.push(`Expected error code ${expect.errorCode}, got ${response.error.code}`);
            }
        }
        // Check error data code
        if (expect.errorDataCode !== undefined && response.error?.data) {
            const dataCode = response.error.data.code;
            if (dataCode !== expect.errorDataCode) {
                errors.push(`Expected error.data.code "${expect.errorDataCode}", got "${dataCode}"`);
            }
        }
