 ...
 */
async function handleClientMessage(message, upstream, cloud, pendingRequests, options) {
    ...
    const meta = paramsAny._meta;
    const argsAny = args;
    const approvalToken = meta?.approvalToken ||
        argsAny.approvalToken;
    ...
    try {
        ...
        if (response.decision === "allowed") {
            ...
            if (options.waitForApproval && response.approval_request_id) {
                ...
                const approvalResult = await cloud.waitForApproval(response.approval_request_id, options.approvalTimeoutMs || 300000);
        ...
        const isRiskyAction = classification.actionClass === "execute" ||
 ...
 */
function forwardToUpstream(message, upstream) {
 ...
 */
function createErrorResponse(id, decision, reason) {
 ...
 */
function getDefaultDecisionLocal(classification) {
